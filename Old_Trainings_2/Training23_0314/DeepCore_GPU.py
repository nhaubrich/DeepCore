...
def loss_mse_select_clipped(y_true, y_pred) :
    ...
    pred = y_pred[:,:,:,:,:-1]
    true =  y_true[:,:,:,:,:-1]
    norm = tf.constant([1/2, 1/3, 1/8, 1/6, 1], dtype=tf.float32) # scalar to normalize loss functiom parameters
    wei = wei*norm
    pred = pred*norm
    true = true*norm
    out =K.square(tf.clip_by_value(pred-true,-1,1))*wei # clipping adjusted to 1
...
#######################################################################################################################
    ...
    anubi = keras.optimizers.Adam(learning_rate=0.000002)  #10-6
